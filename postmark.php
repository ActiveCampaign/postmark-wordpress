...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	...
	include plugin_dir_path( __FILE__ ) . 'class-postmarkplugincli.php';
...
class Postmark_Mail {
	...
	public function postmark_load_more_logs() {
		...
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'postmark_nonce' ) ) {
			wp_die( 'We were unable to verify this request, please reload the page and try again.' );
		}

		// Retrieves more logs from logs table using offset and prepare() to prevent SQL injections.
		$result = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM %s ORDER BY log_entry_date DESC LIMIT %d OFFSET %d', $table, 10, $_POST['offset'] ) );
		...
		foreach ( $result as $row ) {
			$new_rows_html = $new_rows_html . '<tr><td align=\"center\">' . date( 'Y-m-d h:i A', strtotime( $row->log_entry_date ) ) .
			'</td><td align=\"center\">' . $row->fromaddress .
			'</td><td align=\"center\">' . $row->toaddress .
			'</td><td align=\"center\">' . $row->subject .
			'</td><td align=\"center\">' . $row->response . '</td></tr>';
		}
	...
	public function send_test_email() {
		...
		if ( isset( $_POST['email'] ) && is_email( $_POST['email'] ) ) {
			...
			wp_die( 'You need to specify a valid recipient email address.', 'postmark-wordpress' );
		}
		...
		if ( false !== $response ) {
			...
			echo 'Test failed, the following is the error generated when running the test send:<br/><pre class="diagnostics">' . esc_html( $dump ) . '</pre>';
		}
	...
	public function save_settings() {
		...
		if ( ! is_array( $data ) ) {
			wp_die( 'Something went wrong!', 'postmark-wordpress' );
		}
...
function pm_clear_old_logs() {
	...
	$rows_to_delete_count = $wpdb->get_var(
		$wpdb->prepare(
			'SELECT COUNT(*) FROM %1$s WHERE %2$s < UNIX_TIMESTAMP( DATE_SUB( NOW(), INTERVAL %3$d DAY) ) LIMIT 500',
			$table_name,
			'log_entry_date',
			7
		)
