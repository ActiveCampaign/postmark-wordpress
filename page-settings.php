...
<div class="wrap">
	...
	<h1 class="nav-tab-wrapper">
		...
		<?php if ( isset( $this->settings['enable_logs'] ) && 1 === $this->settings['enable_logs'] ) : ?>
	...
	<?php if ( isset( $this->settings['enable_logs'] ) && 1 === $this->settings['enable_logs'] ) : ?>
		<div class="tab-content tab-log">
			...
			$count = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM %1$s', $table ) );
			...
			if ( $count > 0 ) {
				...
				$result = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM %1$s ORDER BY log_entry_date DESC LIMIT %2$d', $table, 10 ) );
				...
				foreach ( $result as $row ) {
					...
					esc_html( date( 'Y-m-d h:i A', strtotime( esc_html( $row->log_entry_date ) ) ) ) .
					'</td><td align="center">  ' . esc_html( $row->fromaddress ) . '</td><td align="center">  ' . esc_html( $row->toaddress ) . '</td><td align="center">  ' . esc_html( $row->subject ) . '</td><td align="center">  ' . esc_html( $row->response ) . '</td></tr>';
				}
